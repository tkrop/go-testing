...
// GetMock resolves the related mock from the mock handler, if the provided
// value is a mock constructor. Else the provided value is returned as is. This
// can be used to resolve optional mock expectations without breaking other
// test expectations.
func (mocks *Mocks) GetMock(expect any) any {
	...
	ctype := reflect.TypeOf((*gomock.Controller)(nil))
	if !ftype.In(0).AssignableTo(ctype) {
		return expect
	}

	return mocks.Get(value)
}
